...
from billing.constants import BillingInterval, DocumentTypes, PaymentProviders
...
from billing.services.provider_credentials import PublicProviderCredentials
from billing.virtual_models import (
...
class LedgerEventSerializer(serializers.Serializer):
    ...
    straight from :meth:`billing.metering.OccurrenceSource.describe`. Declared
    ...
    id = serializers.IntegerField(help_text="The occurrence's external id.")
...
class MeteredOccurrenceSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance: MeteredOccurrence):
        ...
        # `describe()` need not echo the id back, and a project's mapping is
        # keyed by it anyway -- so it is added here rather than being a
        # requirement on every source implementation.
        instance._event_row = (  # type: ignore[attr-defined]
            {"id": instance.event_id, **detail} if detail is not None else None
        )

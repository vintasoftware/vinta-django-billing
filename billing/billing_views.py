...
member; purchase/change actions require ``IsBillingManager``
...
from billing.pagination import LargeLimitOffsetPagination
from billing.permissions import IsBillingManager
...
if TYPE_CHECKING:
    from billing.services.entitlement_service import EntitlementService
...
class MeteredOccurrenceViewSet(TenantScopedViewMixin, mixins.ListModelMixin, GenericViewSet):
    ...
    **Stricter than every other read in this module.** ``BillingUsageViewSet``
    ...
    permission_classes = (IsAuthenticated, IsBillingManager)
    ...
    def _resolve_events(
        self, occurrences: Sequence[MeteredOccurrence]
    ) -> dict[int, dict[str, object]]:
        """Batched, per-page detail for the occurrences on this page.

        ``event_id`` is a soft reference (a ``BigIntegerField``, not a foreign
        key -- see the ``MeteredOccurrence`` model docstring), so detail can
        never come from a ``select_related`` off the ledger itself. It comes
        from the project's configured occurrence source instead, in one batched
        call per page rather than a lookup per row.

        Ids the source cannot resolve -- a deleted underlying object -- are
        absent from the map, and the serializer renders ``event: null`` for
        those rows.
        """
        event_ids = sorted({occurrence.event_id for occurrence in occurrences})
        if not event_ids:
            return {}
        return get_occurrence_source().describe(event_ids)
...
class SubscriptionViewSet(TenantScopedViewMixin, GenericVirtualModelViewMixin, GenericViewSet):
    ...
    def get_permissions(self):
        if self.action in self.write_actions:
            return [IsAuthenticated(), IsBillingManager()]

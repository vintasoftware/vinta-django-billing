...
from billing.models import BillingPlan as BillingPlanModel
from billing.models import BillingProfile as BillingProfileModel
from billing.models import Payment as PaymentModel
from billing.models import PaymentStatusUpdate as PaymentStatusUpdateModel
from billing.models import ProviderWebhookEvent as ProviderWebhookEventModel
from billing.models import Refund as RefundModel
from billing.models import RefundStatusUpdate, SubscriptionStatusUpdate
from billing.models import Subscription as SubscriptionModel
...
# Declared with ``TypeVar`` rather than PEP 695 ``class PaymentService[...]``
# syntax: that spelling is 3.12+, and this package supports 3.11.
PaymentAdapter = TypeVar("PaymentAdapter", bound=BasePaymentAdapter)
SubscriptionAdapter = TypeVar("SubscriptionAdapter", bound=BaseSubscriptionAdapter)
SubscriptionPlanFactory = TypeVar("SubscriptionPlanFactory", bound=BaseSubscriptionPlanFactory)


class PaymentService(Generic[PaymentAdapter, SubscriptionAdapter, SubscriptionPlanFactory]):
    def __init__(
        self,
        subscription_plan_factory: SubscriptionPlanFactory | None = None,
        ...
        if payment_provider_registry is None:
            payment_provider_registry = get_payment_provider_registry()
        if subscription_provider_registry is None:
            subscription_provider_registry = get_subscription_provider_registry()
    ...
    def create_refund(
        ...
        try:
            ...
            RefundStatusUpdate.objects.create(
                ...
                description=(
                    f"Refund created in the payment gateway with status {refund_result.status}"
                ),
            )

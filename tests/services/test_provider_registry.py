...
class TestProviderRegistries:
    """Both shipped providers are reachable without any container wiring.

    The application this was extracted from resolved these through a
    ``dependency_injector`` container. A library should not force a DI framework
    on its host, so the registries are plain module-level functions -- and these
    assert that swap did not quietly drop a provider.
    """
    ...
    def test_payment_service_resolves_both_registries_by_default(self):
        """Constructed with no arguments, the service still finds every adapter.
        ...
        assert set(service.payment_provider_registry) == {
            PaymentProviders.STRIPE,
            PaymentProviders.MERCADOPAGO,
        }
        assert set(service.subscription_provider_registry) == {
            PaymentProviders.STRIPE,
            PaymentProviders.MERCADOPAGO,
        }

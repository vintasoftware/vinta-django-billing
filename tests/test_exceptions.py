...
class TestOverLimitErrorIsFrozen:
    ...
    def test_as_error_body_returns_the_exact_six_key_dict(self):
        """The wire format is frozen: clients branch on these keys."""
        error = OverLimitError(
    ...
    def test_detail_falls_back_to_the_raw_key_for_an_unregistered_resource(self):
        """An unregistered key must not raise while an error is being built.

        Registration is import-time state, so a key can outlive the app that
        registered it -- a queued job, a stale plan row. Rendering the raw key
        is a usable message; a KeyError here would turn a 4xx into a 500.
        """
        error = OverLimitError(
            resource_key="never_registered",
            current_usage=1,
            limit=1,
            remedy="upgrade_plan",
        )

...
class TestEffectiveLimit:
    def test_reads_the_ceiling_off_the_subscription(self, entitlement_service, scope, subscription):
        limit = entitlement_service.get_effective_limit(scope, "widgets")

        assert limit.limit_value == 3
        assert limit.is_unlimited is False

    def test_a_null_limit_is_unlimited_not_zero(
        self, entitlement_service, scope, unlimited_plan, make_subscription
    ):
        """Treating NULL as zero would turn a data gap into a total lockout."""
        make_subscription(scope, unlimited_plan)

        limit = entitlement_service.get_effective_limit(scope, "widgets")

        assert limit.is_unlimited is True

    def test_a_missing_limit_row_is_unlimited(self, entitlement_service, scope, subscription):
        ...
        assert entitlement_service.get_effective_limit(scope, "widgets").is_unlimited

    def test_no_subscription_is_unlimited(self, entitlement_service, scope):
        assert entitlement_service.get_effective_limit(scope, "widgets").is_unlimited


class TestUsage:
    def test_counts_through_the_registered_counter(self, entitlement_service, scope, subscription):
        Widget.objects.create(scope=scope, name="a")
        Widget.objects.create(scope=scope, name="b")

        assert entitlement_service.get_current_usage(scope, "widgets") == 2

    def test_usage_is_isolated_per_organization(
        self, entitlement_service, scope, other_scope, subscription
    ):
        Widget.objects.create(scope=other_scope, name="theirs")

        assert entitlement_service.get_current_usage(scope, "widgets") == 0

    def test_a_two_table_counter_sums_both_sides(
        self, entitlement_service, scope, subscription, user
    ):
        """`seats` counts real seats plus still-open invitations."""
        Seat.objects.create(scope=scope, user=user)
        SeatInvitation.objects.create(scope=scope, email="new@example.com")

        assert entitlement_service.get_current_usage(scope, "seats") == 2
    ...
    def test_extra_reaches_the_projects_counter(
        ...
        """
        Seat.objects.create(scope=scope, user=user)
        invitation = SeatInvitation.objects.create(scope=scope, email="new@example.com")

        usage = entitlement_service.get_current_usage(
            scope, "seats", usage_extra={EXCLUDE_INVITATION_ID: invitation.pk}
        )

        assert usage == 1

    def test_an_unregistered_resource_reports_zero_rather_than_raising(
        self, entitlement_service, scope, subscription
    ):
        """Fails open: a stale plan row must not 500 an unrelated request."""
        assert entitlement_service.get_current_usage(scope, "never_registered") == 0


class TestCheckLimit:
    def test_allows_a_create_under_the_ceiling(self, entitlement_service, scope, subscription):
        result = entitlement_service.check_limit(scope, "widgets", delta=1)
    ...
    def test_allows_filling_the_last_unit_of_capacity(
        ...
        assert entitlement_service.check_limit(scope, "widgets", delta=1).allowed is True
    ...
    def test_reports_the_registered_remedy_on_refusal(
        ...
        SeatInvitation.objects.create(scope=scope, email="a@example.com")
        SeatInvitation.objects.create(scope=scope, email="b@example.com")

        result = entitlement_service.check_limit(scope, "seats", delta=1)
    ...
    def test_unlimited_never_counts_usage(
        ...
        result = entitlement_service.check_limit(scope, "widgets", delta=1000)
...
class TestAddOns:
    def test_an_active_add_on_raises_the_ceiling(self, entitlement_service, scope, subscription):
        ...
        assert entitlement_service.get_effective_limit(scope, "widgets").limit_value == 5

    def test_an_inactive_add_on_does_not(self, entitlement_service, scope, subscription):
        from vinta_billing.models import SubscriptionAddOn
        ...
        assert entitlement_service.get_effective_limit(scope, "widgets").limit_value == 3
...
class TestLazyUsageExtra:
    ...
    is paying for an answer nobody reads. Every scope is unlimited for the
    ...
    def test_it_matches_the_eager_argument_exactly(
        ...
        eager = entitlement_service.check_limit(scope, "seats", usage_extra=extra)
        lazy = entitlement_service.check_limit(scope, "seats", usage_extra_resolver=lambda: extra)
    ...
    def test_it_is_called_exactly_once(self, entitlement_service, scope, subscription, user):
        ...
        entitlement_service.check_limit(scope, "seats", usage_extra_resolver=resolver)
    ...
    def test_it_is_still_called_when_the_check_refuses(
        ...
        result = entitlement_service.check_limit(
            scope, "seats", usage_extra_resolver=lambda: calls.append(1) or {}
        )
    ...
    def test_it_is_not_called_for_a_restricted_billing_root(
        self, entitlement_service, scope, plan, make_subscription
    ):
        ...
        make_subscription(scope, plan, billing_state=BillingState.RESTRICTED)
        calls = []
    ...
    def test_passing_both_raises(self, entitlement_service, scope, subscription):
...
class TestInapplicableUsageExtra:
    ...
    def test_a_key_the_resource_does_not_read_is_refused(
        ...
        with pytest.raises(InapplicableUsageExtraError) as excinfo:
            entitlement_service.check_limit(
                scope, "event_occurrences", usage_extra={EXCLUDE_INVITATION_ID: 1}
            )

        assert excinfo.value.resource_key == "event_occurrences"
        assert excinfo.value.unexpected_keys == [EXCLUDE_INVITATION_ID]

    def test_a_declared_key_is_allowed(self, entitlement_service, scope, subscription, user):
        invitation = SeatInvitation.objects.create(scope=scope, email="new@example.com")
    ...
    def test_a_typo_on_a_declared_resource_is_refused(
        ...
        with pytest.raises(InapplicableUsageExtraError):
            entitlement_service.check_limit(scope, "seats", usage_extra={"exclude_invite_id": 1})
    ...
    def test_an_undeclared_resource_still_takes_anything(
        ...
        result = entitlement_service.check_limit(
            scope, "widgets", usage_extra={"anything at all": 1}
        )
    ...
    def test_it_is_refused_on_the_unlimited_path_too(
        ...
        Every scope is unlimited during a rollout -- exactly when a call
    ...
    def test_a_resolvers_result_is_checked_the_same_way(
        ...
        with pytest.raises(InapplicableUsageExtraError):
            entitlement_service.check_limit(
                scope, "seats", usage_extra_resolver=lambda: {"exclude_invite_id": 1}
            )
    ...
    def test_the_read_methods_are_held_to_the_same_rule(
        ...
        with pytest.raises(InapplicableUsageExtraError):
            entitlement_service.get_current_usage(
                scope, "seats", usage_extra={"exclude_invite_id": 1}
            )
        with pytest.raises(InapplicableUsageExtraError):

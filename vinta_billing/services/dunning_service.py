...
service** (the webhook handlers in ``vinta_billing/views.py``, the beat task in
``vinta_billing/jobs.py``) rather than writing ``Subscription.billing_state``
...
class DunningService:
    """Stateless; built by ``vinta_billing.services.container``.
    ...
    def process_subscription(self, subscription: Subscription) -> None:
        ...
        docstring. Re-reads nothing: the caller (``vinta_billing.jobs.

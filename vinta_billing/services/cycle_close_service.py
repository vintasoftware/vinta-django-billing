...
5. **Best-effort across subscriptions.** One subscription's close failing (a
   ...
   its own failure (``vinta_billing.jobs.close_subscription_billing_period``).
...
from vinta_billing.services.billing_dataclasses import ClosedPeriod, ReconciliationReport
from vinta_billing.services.billing_state_machine import transition_billing_state
from vinta_billing.services.entitlement_service import (
    EntitlementService,
...
from vinta_billing.services.subscription_service import SubscriptionService, overage_settlement_step
...
class CycleCloseService:
    ...
    Stateless; built by ``vinta_billing.services.container``. Runs from a background job,
    ...
    def __init__(
        ...
        subscription_service = subscription_service or container.get_subscription_service()
        payment_service = payment_service or container.get_payment_service()
        entitlement_service = entitlement_service or container.get_entitlement_service()
            ...
            with transaction.atomic():
                ...
                for resource_key in resources.keys():
                    ...
                    if resource_key == metered_resource_key():
                        ...
                        total = sum(usage_breakdown.values()) if resource_key in resources else None

"""Resolves which payment provider governs an scope's *future* charges.
...
scope's provider -- so the pin -> default resolution rule cannot drift between the
...
from vinta_billing.models import AbstractBillingScope, BillingProfile
...
class PaymentProviderResolver:
    """Resolves the payment provider governing an scope's next charge.

    Stateless -- reads ``VINTA_BILLING['DEFAULT_PROVIDER']`` and the scope's own
    ``BillingProfile.payment_provider`` pin. No adapter or secret credential is reachable
    from this class.
    """

    def resolve_for_scope(self, scope: AbstractBillingScope) -> str:
        """The provider governing ``scope``'s next charge: its pin when non-empty,
        ``VINTA_BILLING['DEFAULT_PROVIDER']`` otherwise.

        An scope with no ``BillingProfile`` at all resolves to the default, exactly

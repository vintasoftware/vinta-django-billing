...
from vinta_billing.constants import BillingInterval, BillingState, PaymentProviders
...
class SubscriptionService:
    ...
    def retry_failed_charge(self, subscription: Subscription, idempotency_key: str) -> Subscription:
        ...
        ``vinta_billing/jobs.py::process_dunning``).
        ...
        **Nor must a declined (or unattemptable) charge.** ``ChargeDeclinedError``
        ...
        (``vinta_billing.jobs.process_dunning_for_subscription``) unhandled: per
    ...
    def set_payment_provider(
        ...
        ``vinta_billing.admin.BillingProfileAdmin.save_model``, which catches nothing).
        ...
        # Sent inside the caller's transaction, so a project whose receiver
        # writes an audit row gets the row and the repoint committed together --
        # or neither.

...
otherwise touch a payment adapter (``vinta_billing.services.payment_adapters`` /
``vinta_billing.services.subscription_adapters``) or read any *secret* key out of that same
...
(``vinta_billing.views.PaymentProviderViewSet``), so it must have no code path that could ever

...
def _registry_label(registry, key: str) -> str:
    """The registered label for ``key``, lowercased, or the key itself.

    Never raises: this runs while an error is already being built, and a
    KeyError here would replace a useful 4xx with a 500.
    """
    if key in registry:
        return str(registry.get(key).label).lower()
    return key
...
class BillingError(Exception):
    ...
    #: Subclasses rendered by ``vinta_billing.exception_handling`` must override it.
...
class OverLimitError(BillingError):
    ...
    def build_detail(resource_key: str) -> str:
        ...
        Uses the registered label when the key is known and falls back to the
        raw key otherwise, so an unregistered key degrades to a usable message
    ...
    def from_missing_entitlement(cls, entitlement_key: str) -> "OverLimitError":
        ...
        error, and every renderer -- ``vinta_billing.exception_handling`` and whatever
        ...
        """
        label = _registry_label(entitlements, entitlement_key)
        return cls(
...
class IllegalBillingStateTransitionError(BillingError):
    """Raised by ``vinta_billing.services.billing_state_machine.transition_billing_state``
...
class CollectionNotSupportedError(PaymentAdapterError):
    ...
    branch in ``vinta_billing.exception_handling``, so on its own it would reach
...
class ChargeDeclinedError(PaymentError):
    ...
    ``vinta_billing.exception_handling`` branch, instead of a clean typed response.
    """
